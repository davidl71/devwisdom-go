...
func TestNewLogger(t *testing.T) {
	...
	// Test DEBUG level when DEVWISDOM_DEBUG=1
	os.Setenv("DEVWISDOM_DEBUG", "1")
	logger = NewLogger()
	if logger.level != LevelDebug {
		t.Errorf("Expected DEBUG level when DEVWISDOM_DEBUG=1, got %v", logger.level)
	}
	os.Unsetenv("DEVWISDOM_DEBUG")
}
...
func TestLogLevel_String(t *testing.T) {
	...
	for _, tt := range tests {
		if got := tt.level.String(); got != tt.expected {
			t.Errorf("LogLevel(%d).String() = %q, want %q", tt.level, tt.expected, got)
		}
	}
}
